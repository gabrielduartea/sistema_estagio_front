...
<template>
    <v-container>
        ...
        <v-col>
            <h1 class="title">Relatório de estágios por empresa</h1>
        </v-col>
        <v-card-text>
            <v-container>
                <v-row>
                    <v-col cols="12" sm="6" md="6">
                        <v-autocomplete v-model="editedItem.empresaId" label="Empresa" :items="itemsEmpresas"
...
export default {
    ...
    methods: {
        ...
        async getEmpresas() {
            const empresas = await axios.get(`${baseURL}empresas/findAll`)
        ...
        async gerarRelatorio() {
            debugger
            const infoRelatorio = []
            const dadosRelatorio = (await axios.get(`${baseURL}estagios/relatorioEstagiosPorEmpresa/${JSON.stringify(this.editedItem)}`)).data
